...
interface Copy {
  ...
  bodyHtmlOffer: (moduleCount: number) => string;
  ctaActive: string;
  ctaOffer: string;
}
...
const COPY: Record<Locale, Copy> = {
  pt: {
    ...
    bodyHtmlOffer: (count) => `
      <p style="margin:0 0 12px;">É um prazer ter você na <strong>Zelo BJJ</strong>.</p>
      <p style="margin:0 0 12px;">Sua conta está pronta. Para liberar todas as aulas, garanta o acesso ao curso completo. São ${count} módulos estruturados para você evoluir do zero ao avançado.</p>
    `,
  ...
  en: {
    ...
    bodyHtmlOffer: (count) => `
      <p style="margin:0 0 12px;">It's a pleasure to have you at <strong>Zelo BJJ</strong>.</p>
      <p style="margin:0 0 12px;">Your account is ready. To unlock all lessons, secure your access to the full course. ${count} structured modules will take you from zero to advanced.</p>
    `,
  ...
  ko: {
    ...
    bodyHtmlOffer: (count) => `
      <p style="margin:0 0 12px;"><strong>Zelo BJJ</strong>에 오신 것을 진심으로 환영합니다.</p>
      <p style="margin:0 0 12px;">계정이 준비되었습니다. 모든 강의를 잠금 해제하려면 전체 강좌에 대한 접근 권한을 확보하세요. 초보부터 고급까지 ${count}개의 체계적인 모듈로 구성되어 있습니다.</p>
    `,
    ctaActive: "학습 시작하기",
    ctaOffer: "접근 권한 받기",
  },
...
export function welcomeTemplate(params: WelcomeTemplateParams): EmailRender {
  const { locale, fullName, hasActiveSubscription, moduleCount } = params;
  ...
  const effectiveCount = moduleCount && moduleCount > 0 ? moduleCount : 11;
  const bodyHtml = hasActiveSubscription
    ? copy.bodyHtmlActive
    : copy.bodyHtmlOffer(effectiveCount);
